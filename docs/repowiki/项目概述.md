...
- [main.go](file://main.go) -- 程序入口、Swagger 注解、信号处理
- [internal/app/app.go](file://internal/app/app.go) -- 应用初始化流程（`Init` / `Start` / `Close`）
- [internal/app/routers.go](file://internal/app/routers.go) -- 路由注册与中间件装配
- [internal/config/types.go](file://internal/config/types.go) -- 启动配置结构定义
- [internal/version/version.go](file://internal/version/version.go) -- 版本号注入与输出
- [go.mod](file://go.mod) -- Go 模块声明与依赖列表
- [Makefile](file://Makefile) -- 构建、测试、部署命令
...
Songloft 是一款**自托管的本地音乐服务器**，旨在让用户在自有硬件上统一管理和播放本地音乐与网络音源。项目当前版本为 **2.8.0**，采用 Apache 2.0 许可证开源。
...
**章节来源**
- [main.go:26-38](file://main.go#L26-L38) -- Swagger 注解中的项目描述、版本与默认端口
- [internal/version/version.go:1-24](file://internal/version/version.go#L1-L24) -- 版本号定义
...
**章节来源**
- [go.mod:1-3](file://go.mod#L1-L3) -- 模块名称与 Go 版本
- [internal/app/app.go:34-58](file://internal/app/app.go#L34-L58) -- App 结构体字段一览（展示各核心组件）
...
- **权限模型** -- manifest 声明 `permissions`（`network`/`storage`/`fs:music` 等），运行时校验。
...
**章节来源**
- [internal/app/app.go:87-391](file://internal/app/app.go#L87-L391) -- `Init()` 方法中各服务的创建与装配
- [internal/app/app.go:291-323](file://internal/app/app.go#L291-L323) -- 音源处理链装配（Fetcher/Resolver/Orchestrator）
- [internal/app/app.go:378-389](file://internal/app/app.go#L378-L389) -- 自动扫描与 JS 插件异步启动
...
**章节来源**
- [go.mod:5-22](file://go.mod#L5-L22) -- 直接依赖列表
- [go.mod:57](file://go.mod#L57) -- `pkg/tag` 本地替换
...
**图表来源**
- [internal/app/app.go:34-58](file://internal/app/app.go#L34-L58) -- App 结构体展示了各子系统的组合关系
- [internal/app/routers.go:1-35](file://internal/app/routers.go#L1-L35) -- 路由注册流程展示了中间件与 handler 的装配顺序
...
**图表来源**
- [main.go:45-78](file://main.go#L45-L78) -- `main()` 函数：解析配置、初始化、信号处理、启动
- [internal/app/app.go:87-391](file://internal/app/app.go#L87-L391) -- `Init()` 完整初始化流程
...
**章节来源**
- [main.go:12-24](file://main.go#L12-L24) -- 内存软限制与 GC 配置
- [internal/app/app.go:100-112](file://internal/app/app.go#L100-L112) -- 数据库打开与迁移
...
**章节来源**
- [internal/app/app.go:547-637](file://internal/app/app.go#L547-L637) -- `ParseConfig()` 函数：CLI 参数与环境变量解析
- [internal/app/app.go:458-483](file://internal/app/app.go#L458-L483) -- `Start()` 函数：HTTP 服务启动与 BasePath 处理
- [Makefile:97-108](file://Makefile#L97-L108) -- `build` / `build-lite` 目标
- [Makefile:249-252](file://Makefile#L249-L252) -- `run` 目标
- [Makefile:298-305](file://Makefile#L298-L305) -- `docker-build` / `docker-run` 目标
...
- [后端系统设计](后端系统设计/) -- 路由、中间件、数据访问、服务层详细设计
- [核心功能实现](核心功能实现/) -- 扫描、缓存、音源编排、HLS 代理等功能实现
- [插件系统设计](插件系统设计/) -- JS 运行时、插件管理器、权限模型
- [API 接口参考](API%20接口参考/) -- RESTful API 完整文档

**章节来源**
- [main.go:26-28](file://main.go#L26-L28) -- 项目定位描述
- [internal/app/app.go:34-58](file://internal/app/app.go#L34-L58) -- 核心组件一览
