...
## 简介
本文件系统性梳理 Songloft 的 SQLite 数据库模式设计，覆盖整体架构、表结构、关系设计、索引策略、约束定义、触发器、实体关系图（ERD）、数据迁移与版本管理、性能优化与查询技巧，以及数据库初始化脚本与变更最佳实践。目标是帮助开发者与运维人员快速理解并高效维护数据库层。
...
### 索引策略设计
- 单列索引
  - songs: type, title, artist, added_at(倒序)
...
- UNIQUE：configs.key、auth_tokens.token_id
...
### 歌词存储格式标准化
**新增** Songloft 现在采用统一的 LyricPayload JSON 格式存储歌词：
...
- 常见错误与定位
  ...
  - 重复键冲突：UNIQUE 冲突（如 token_id/key），检查唯一性
...
## 结论
Songloft 的数据库层以清晰的接口抽象、完善的约束与索引策略、合理的触发器与迁移机制，构建了稳定高效的本地音乐管理能力。通过事务与批量优化、WAL 并发与连接池配置，兼顾了易用性与性能。**新增的歌词JSON标准化和URL分离存储机制**进一步增强了系统的灵活性和性能表现。建议在后续演进中持续关注查询模式与热点字段，动态调整索引与分区策略，并将复杂迁移纳入自动化流程。
